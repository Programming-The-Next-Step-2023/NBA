...
#'
#' @param matchup based on NBA game log data, in the format "XXX ad YYY" using
#' team tri-codes.
#'
...
team_stats <- function(data, game_id, team) {
  ...
  data <- dplyr::select(data, ptsTeam, pctFGTeam, pctFG3Team, pctFTTeam,
                        orebTeam, drebTeam, astTeam, stlTeam, blkTeam, tovTeam,
                        plusminusTeam)
}
...
player_stats <- function(data, game_id, player) {
  ...
  pbpdat <- dplyr::filter(pbpdat, pbpdat$playerNameI == player)
  player_id <- unlist(na.omit(unique(pbpdat$personId)))
  box_data <- nbastatR::box_scores(game_id, box_score_types = "Traditional",
                                   result_types = "player")
  ...
  box_score <- dplyr::select(dataBoxScore, pts, pctFG, pctFG3, pctFT, oreb,
                             dreb, ast, stl, blk, tov, plusminus)
}
...
ui <- shiny::fixedPage(
  ...
  shiny::fixedRow(
    shiny::column(
      2,
      shiny::span(shiny::textOutput("final_score"),
  ...
  shiny::fixedRow(
    ...
    shiny::column(
      2,
      shiny::span(shiny::textOutput("score"),
                  style = "font-size:20px; font-weight:bold")
    ),
    shiny::column(
      3,
...
server <- function(input, output, session) {
  ...
  coordinates <- shiny::reactive(game_coordinates(gamedata(), input$period,
                                                  input$time, input$team,
                                                  input$player))
  ...
  output$description_pbp <- shiny::renderTable({
    ...
    description_pbp_data <- dplyr::select(description_pbp_data, clock,
                                          description, scoreAway, scoreHome)
    colnames(description_pbp_data) <- c("Clock", "Description", "Score Away",
                                        "Score Home")
  ...
  output$team_stats <- shiny::renderTable({
    ...
    colnames(team_st_data) <- c("Points", "Field goal %", "3 point %",
                                "Freethrow %", "Offensive rebounds",
                                "Defensive rebounds", "Assists", "Steals",
                                "Blocks", "Turnovers", "Plus/Minus")
    if (input$team != "both") {
      team_st_data
    }
  })
  ...
  output$player_stats <- shiny::renderTable({
    if (!is.null(player_box()) && input$player != "all") {
      ...
      colnames(player_st_data) <- c("Points", "Field goal %", "3 point %",
                                    "Freethrow %", "Offensive rebounds",
                                    "Defensive rebounds", "Assists", "Steals",
                                    "Blocks", "Turnovers", "Plus/Minus")
      player_st_data
  ...
  output$player_default_picture <- shiny::renderUI({
    if (input$player == "all") {
      tags$img(
        src = "https://upload.wikimedia.org/wikipedia/en/thumb/0/03/National_Basketball_Association_logo.svg/315px-National_Basketball_Association_logo.svg.png?20221026000552",
        alt = "df",
        style = "width: auto; height: 200px;"
      )
...
#' Function that opens the shiny app, this is the only function that is exported
#' and needs to be run to use the app.
